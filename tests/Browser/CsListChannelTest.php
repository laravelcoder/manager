...
class CsListChannelTest extends DuskTestCase
{
    ...
    public function testCreateCsListChannel()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $cs_list_channel) {
            $browser->loginAs($admin)
                ...
                ->select("sync_server_id", $cs_list_channel->sync_server_id)
                ->press('Save')
                ...
                ->assertSeeIn("tr:last-child td[field-key='sync_server']", $cs_list_channel->sync_server->name)
                ->logout();
        });
    }

    public function testEditCsListChannel()
    {
        $admin = \App\User::find(1);
        ...
        $this->browse(function (Browser $browser) use ($admin, $cs_list_channel, $cs_list_channel2) {
            $browser->loginAs($admin)
                ->visit(route('admin.cs_list_channels.index'))
                ...
                ->select("sync_server_id", $cs_list_channel2->sync_server_id)
                ->press('Update')
                ->assertRouteIs('admin.cs_list_channels.index')
                ...
                ->assertSeeIn("tr:last-child td[field-key='sync_server']", $cs_list_channel2->sync_server->name)
                ->logout();
        });
    }

    public function testShowCsListChannel()
    {
        $admin = \App\User::find(1);
        $cs_list_channel = factory('App\CsListChannel')->create();
        ...
        $this->browse(function (Browser $browser) use ($admin, $cs_list_channel) {
            $browser->loginAs($admin)
                ...
                ->assertSeeIn("td[field-key='sync_server']", $cs_list_channel->sync_server->name)
                ->logout();
        });
    }
