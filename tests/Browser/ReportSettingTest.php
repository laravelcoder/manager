...
{
    public function testCreateReportSetting(): void
    {
        $admin = \App\User::find(1);
        $report_setting = factory('App\ReportSetting')->make();

        $this->browse(function (Browser $browser) use ($admin, $report_setting): void {
    ...
    public function testEditReportSetting(): void
    {
        $admin = \App\User::find(1);
        $report_setting = factory('App\ReportSetting')->create();
        $report_setting2 = factory('App\ReportSetting')->make();

        $this->browse(function (Browser $browser) use ($admin, $report_setting, $report_setting2): void {
            $browser->loginAs($admin)
                ->visit(route('admin.report_settings.index'))
    ...
    public function testShowReportSetting(): void
    {
        $admin = \App\User::find(1);
        $report_setting = factory('App\ReportSetting')->create();
