...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped ajaxTable @can('realtime_notification_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
...
@section('javascript') 
    <script>
        ...
        $(document).ready(function () {
            window.dtDefaultOptions.ajax = '{!! route('admin.realtime_notifications.index') !!}?show_deleted={{ request('show_deleted') }}';
            window.dtDefaultOptions.columns = [@can('realtime_notification_delete')
                @if ( request('show_deleted') != 1 )
                    {data: 'massDelete', name: 'id', searchable: false, sortable: false},
                @endif
                @endcan{data: 'server_type', name: 'server_type'},
                {data: 'r_urltn', name: 'r_urltn'},
